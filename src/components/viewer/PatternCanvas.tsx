...
export function PatternCanvas({ cells, width, height, palette, viewMode }: PatternCanvasProps) {
  ...
  useEffect(() => {
    ...
    setOffset({
      x: (rect.width - width * DEFAULT_CELL_PX) / 2,
      y: (rect.height - height * DEFAULT_CELL_PX) / 2,
    })
  }, [width, height])
  ...
  const draw = useCallback(() => {
    ...
    const colEnd = Math.min(width, Math.ceil((rect.width - offset.x) / cellPx))
    const rowStart = Math.max(0, Math.floor(-offset.y / cellPx))
    const rowEnd = Math.min(height, Math.ceil((rect.height - offset.y) / cellPx))
